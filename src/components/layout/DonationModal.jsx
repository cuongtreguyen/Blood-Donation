...
const DonationModal = ({ show, onClose, userData }) => {
  ...
  const validateForm = () => {
    ...
    if (formData.last_donation) {
      ...
      if (diffDays < 84) { // 12 weeks = 84 days
    ...
    if (formData.preferred_date) {
      ...
      // Check if preferred date is too far in future (e.g., more than 3 months)
  ...
  // Simplified error handling - ưu tiên message từ backend
  const handleApiError = (error) => {
    ...
    if (error.response?.data) {
      ...
      // Ưu tiên message chính từ backend
      if (data.message) {
        toast.error(`❌ ${data.message}`);
        return;
      }
      
      // Xử lý trường hợp có errors array hoặc object
      if (data.errors) {
        if (Array.isArray(data.errors)) {
          // Nếu là array: ["Ngày mong muốn hiến máu là bắt buộc", "Email không hợp lệ"]
          data.errors.forEach(message => {
            toast.error(`❌ ${message}`);
          });
        } else if (typeof data.errors === 'object') {
          // Nếu là object: { "wantedDate": ["Ngày mong muốn hiến máu là bắt buộc"] }
