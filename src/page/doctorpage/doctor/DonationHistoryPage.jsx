...
const DonationHistoryPage = () => {
  ...
  setCertificateModalOpen(true);
  form.setFieldsValue({
    // Sử dụng tên của người dùng đang đăng nhập. 
    // Thêm ' || "" ' để phòng trường hợp tên không tồn tại.
    doctorName: currentUser?.fullName || "Bác sĩ phụ trách", 
    notes: "Giấy chứng nhận hiến máu hợp lệ",
  });
};
  ...
  const handleCertificateSubmit = async (values) => {
    if (!selectedDonation || !selectedDonor) {
      message.error("Thiếu thông tin hiến máu hoặc người hiến!");
      setCertificateLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log("certificateData:", certificateData); // Debug dữ liệu gửi đi

      const response = await createCertificate(certificateData);
      console.log("createCertificate response:", response); // Debug phản hồi API

      // Kiểm tra phản hồi dựa trên cấu trúc thực tế
      if (!response || (typeof response === "object" && Object.keys(response).length === 0)) {
        throw new Error("Phản hồi từ server không hợp lệ");
      }
      if (response.error || (response.message && response.message.includes("error"))) {
        throw new Error(response.message || "Lỗi từ server khi tạo chứng nhận");
      }
