...
const HistoryItem = ({ donation, isLast, isDonation = true }) => {
  const handleDownload = async () => {
    ...
    try {
      ...
      element.style.width = '840px'; // Match certificate width
      element.innerHTML = createCertificateHTML(certificateData, donation);
      ...
      // Download the PDF
      pdf.save(`ChungNhanHienMau-${donation.certificateId}-${certificateData.donorName.replace(/\s+/g, '_')}.pdf`);

      // Clean up
      document.body.removeChild(element);
    } catch (error) {
      alert("Lỗi khi tải chứng nhận hiến máu. Vui lòng thử lại sau.");
      console.error("Lỗi khi tải chứng nhận:", error);
    }
  };

  const getDate = () => {
    return isDonation ? donation.completedDate : donation.receiveDate;
  };
...
const HistoryComponent = ({ userId: propUserId }) => {
  ...
  useEffect(() => {
    const fetchHistories = async () => {
      ...
      try {
        const [donationResponse, receiveResponse] = await Promise.all([
          api.get(`/blood-register/history/${userId}`),
          api.get(`/blood-receive/get-list-receive-by-user-id?userId=${userId}`),
        ]);

        setDonationHistory(
          donationResponse.data.sort(
            (a, b) => new Date(b.completedDate) - new Date(a.completedDate)
          )
        );
        setReceiveHistory(
          receiveResponse.data.sort(
  ...
  const renderContent = () => {
    ...
    if (donationHistory.length === 0 && receiveHistory.length === 0)
      return (
        <div className="text-center py-12">
