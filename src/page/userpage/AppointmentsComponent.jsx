...
const EditForm = ({ appointment, onSubmit, onCancel, isLoading, userData }) => {
  const { register, handleSubmit, formState: { errors }, reset } = useForm({
    defaultValues: {
      ...
      wantedHour: appointment.wantedHour ? appointment.wantedHour.split(':')[0] + ':' + appointment.wantedHour.split(':')[1] : "",
  ...
  const handleFormSubmit = async (data) => {
    ...
    if (data.wantedHour) {
      const [hours, minutes] = data.wantedHour.split(':').map(Number);
      if (hours < 8 || hours >= 17) {
        Modal.error({
          title: "Lỗi thời gian",
          content: "Thời gian hẹn phải nằm trong khoảng từ 8:00 sáng đến 5:00 chiều.",
        });
        return;
      }
    }
           ...
           <input
            ...
            {...register("wantedHour", { 
              ...
              validate: (value) => {
                ...
                return (hours >= 8 && hours < 17) || "Thời gian phải từ 8:00 sáng đến 5:00 chiều";
              }
            })}
            type="time"
            className="w-full p-2 border border-red-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"
...
const AppointmentManager = () => {
  ...
  const hasChanges = (newData, existingAppointment) => {
    const formattedHour = newData.wantedHour ? `${newData.wantedHour}:00` : "";
    return (
      newData.wantedDate !== existingAppointment.wantedDate ||
      formattedHour !== (existingAppointment.wantedHour || "") ||
      newData.bloodType !== (existingAppointment.bloodType || "") ||
      newData.emergencyName.trim() !== (existingAppointment.emergencyName || "") ||
      newData.emergencyPhone.trim() !== (existingAppointment.emergencyPhone || "") ||
      (newData.medicalHistory?.trim() || null) !== (existingAppointment.medicalHistory || null)
    );
  };
  ...
  const onSubmitEdit = async (data) => {
    ...
    try {
      ...
      // Check for changes
      if (!hasChanges(data, appointment)) {
        setEditingId(null);
        Modal.info({
          title: "Không có thay đổi",
          content: "Không có thông tin nào được thay đổi.",
        });
        return;
      }

      // Format wantedHour to append :00 for both DONATE and RECEIVE types
      const formattedHour = data.wantedHour ? `${data.wantedHour}:00` : "";
